          ...
          <form class="space-y-6" v-on:submit.prevent="submit" :action="currentUri" method="POST">
            ...
            <div class="grid gap-y-6 gap-x-4 sm:grid-cols-6">
              ...
              <div v-if="data.includes('name')" :class="data.includes('state')?'sm:col-span-5':'sm:col-span-6'">
                <text-input :reset-server-error="resetServerError" :fields="fields" field="name">Name</text-input>
              ...
              <div v-if="data.includes('brand')" class="sm:col-span-3">
                <text-input :reset-server-error="resetServerError" :fields="fields" field="brand">Brand</text-input>
                <error :field="v.fields.brand" :server-error="serverErrors.brand"></error>
              </div>

              <div v-if="data.includes('plate')" class="sm:col-span-3">
                <text-input :reset-server-error="resetServerError" :fields="fields" field="plate" :vm="['AAA-#N##']">Licence Plate</text-input>
                <error :field="v.fields.plate" :server-error="serverErrors.plate"></error>
              </div>

              <div v-if="data.includes('color')">
                <text-input :reset-server-error="resetServerError" :fields="fields" field="color">Color</text-input>
              ...
              <div v-if="data.includes('cnpj')" class="sm:col-span-3">
                <text-input :reset-server-error="resetServerError" :fields="fields" field="cnpj" :vm="['##.###.###/####-##']">CNPJ</text-input>
                <error :field="v.fields.cnpj" :server-error="serverErrors.cnpj"></error>
              </div>

              <div v-if="data.includes('cpf')" class="sm:col-span-3">
                <text-input :reset-server-error="resetServerError" :fields="fields" field="cpf" :vm="['###.###.###-##']">CPF</text-input>
                <error :field="v.fields.cpf" :server-error="serverErrors.cpf"></error>
              </div>

              <div v-if="data.includes('birth-date')" class="sm:col-span-3">
                <text-input :reset-server-error="resetServerError" :fields="fields" field="birth-date" :vm="['##/##/####']">Birth Date</text-input>
              ...
              <div v-if="data.includes('phone-number')" class="sm:col-span-6">
                <text-input :reset-server-error="resetServerError" :fields="fields" field="phone-number" :vm="['(##) ####-####', '(##) #####-####']">Phone Number</text-input>
            ...
            <div v-if="data.includes('email')">
              <text-input :reset-server-error="resetServerError" :fields="fields" field="email">Email Address</text-input>
              <error :field="v.fields.email" :server-error="serverErrors.email"></error>
            </div>
        
            <div v-if="data.includes('password')">
              <text-input :reset-server-error="resetServerError" :fields="fields" field="password">Password</text-input>
              <error :field="v.fields.password"></error>
            </div>

            <div v-if="data.includes('confirm-password')">
              <text-input :reset-server-error="resetServerError" :fields="fields" field="confirm-password">Confirm Password</text-input>
...
<script>
export default {
    props: ['states', 'cities', 'defineCities', 'submit', 'fields', 'v', 'serverErrors', 'hrefSubtitle', 'data', 'success', 
      'logo', 'resetServerError'],
    data() {
      return {
