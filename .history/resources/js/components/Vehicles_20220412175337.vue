...
export default {
    ...
    watch: {
      plate(newPlate, oldPlate) {
        let plate;
        plate = this.plate.replace(/\w/g, e => e[0].toUpperCase());
        plate = this.plate.replace(/[A-Z]{4}/g, e => e.substring(0, 3) + '-');
        plate = this.plate.replace(/-[A-Z]/g, e => e.substring(0, -1));
        plate = this.plate.replace(/^-/g, e => e.substring(0, -1));
        plate = this.plate.replace(/[^\w-]/g, e => e.substring(0, -1));
        this.plate = this.plate.replace(/\d[A-Z]{1}/g, e => e.substring(0, -1));
      }
    },
    computed:{
      updatedVehicles(){
