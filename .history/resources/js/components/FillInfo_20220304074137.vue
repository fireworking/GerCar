...
const phoneNumber = (value) => value.length == 14 || value.length == 14;
export default {
  setup () {
  ...
  validations () {
    return {
      fields: {
        ...
        birthDate: { required, date: helpers.withMessage('Must be a valid date', date) },
        phoneNumber: { required, phoneNumber: helpers.withMessage('Must be a valid phone number', phoneNumber) }
      }
    }
  }
}
