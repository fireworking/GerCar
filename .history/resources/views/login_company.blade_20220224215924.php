...
<register-company v-slot="{states, cities, defineCities, submit, fields, v, serverErrors}" v-cloak>
    <default-form :states="states" :cities="cities" :define-cities="defineCities" :submit="submit" 
      ...
      :data="['name', 'state', 'cnpj', 'city', 'email', 'password']">
    ...
    {{-- <register-company v-slot="{states, cities, defineCities, submit, fields, v, serverErrors}" v-cloak>
        <div class="min-h-full flex flex-col justify-center py-16 sm:py-20 sm:px-16 lg:px-20">
            ...
            <div class="sm:mt-2 sm:mx-auto sm:w-full sm:max-w-xl">
                <div class="bg-white pt-2 pb-4 sm:pb-4 sm:pt-1 px-4 shadow sm:rounded-lg sm:px-10">
                <form class="space-y-6" v-on:submit.prevent="submit" action="/login-company" method="POST">
                    ...
                    <div>
                        <button id="register" type="submit" class="w-full mb-4 mt-8 flex justify-center py-3 sm:py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-orange-600 hover:bg-orange-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-500">
                            Login
                        </button>
                    </div>
                </form>
