<template>
    ...
    <div :class="'min-h-full flex flex-col justify-center ' + (logoValue ? 'py-16 ' : 'py-8 ') + 'sm:px-16 lg:px-20'">
      <div class="sm:mx-auto sm:w-full sm:max-w-md">
        <a href=""><img v-if="logoValue" src="img/car.png" class="w-20 m-auto p-2 border-2 rounded-full border-orange-500">   </a>   
        ...
        <div class="bg-white py-2 sm:pb-4 sm:pt-1 px-4 shadow sm:rounded-lg sm:px-10">
          <form class="space-y-6" v-on:submit.prevent="submit" :action="currentUri" method="POST">
            ...
            <div class="grid gap-y-6 gap-x-4 sm:grid-cols-6">
              ...
              <div v-if="data.includes('name')" :class="data.includes('state')?'sm:col-span-5':'sm:col-span-6'">
                <text-input :server-errors="serverErrors" :fields="fields" field="name">Name</text-input>
                <error :field="v.fields.name" :server-error="serverErrors.name"></error>
                <error v-if="data.includes('state')" :field="v.fields.state"></error>
              </div>

              <div v-if="data.includes('brand')" class="sm:col-span-3">
                <text-input :server-errors="serverErrors" :fields="fields" field="brand">Brand</text-input>
                <error :field="v.fields.brand" :server-error="serverErrors.brand"></error>
              </div>

              <div v-if="data.includes('plate')" class="sm:col-span-3">
                <text-input :server-errors="serverErrors" :fields="fields" field="plate" :vm="['AAA-####']">Licence Plate</text-input>
                <error :field="v.fields.plate" :server-error="serverErrors.plate"></error>
              </div>

              <div v-if="data.includes('color')" class="sm:col-span-6">
                <text-input :server-errors="serverErrors" :fields="fields" field="color">Color</text-input>
                <error :field="v.fields.color" :server-error="serverErrors.color"></error>
              </div>
              ...
              <div v-if="data.includes('cnpj')" class="sm:col-span-3">
                <text-input :server-errors="serverErrors" :fields="fields" field="cnpj" :vm="['##.###.###/####-##']">CNPJ</text-input>
                <error :field="v.fields.cnpj" :server-error="serverErrors.cnpj"></error>
              </div>

              <div v-if="data.includes('cpf')" class="sm:col-span-3">
                <text-input :server-errors="serverErrors" :fields="fields" field="cpf" :vm="['###.###.###-##']">CPF</text-input>
                <error :field="v.fields.cpf" :server-error="serverErrors.cpf"></error>
              </div>

              <div v-if="data.includes('birth-date')" class="sm:col-span-3">
                <text-input :server-errors="serverErrors" :fields="fields" field="birth-date" :vm="['##/##/####']">Birth Date</text-input>
                <error :field="v.fields.birthDate" :server-error="serverErrors.birthDate"></error>
              </div>

              <div v-if="data.includes('phone-number')" class="sm:col-span-6">
                <text-input :server-errors="serverErrors" :fields="fields" field="phone-number" :vm="['(##) ####-####', '(##) #####-####']">Phone Number</text-input>
            ...
            <div v-if="data.includes('email')">
              <text-input :server-errors="serverErrors" :fields="fields" field="email">Email Address</text-input>
              <error :field="v.fields.email" :server-error="serverErrors.email"></error>
            </div>
        
            <div v-if="data.includes('password')">
              <text-input :server-errors="serverErrors" :fields="fields" field="password">Password</text-input>
              <error :field="v.fields.password"></error>
            </div>

            <div v-if="data.includes('confirm-password')">
              <text-input :server-errors="serverErrors" :fields="fields" field="confirm-password">Confirm Password</text-input>
