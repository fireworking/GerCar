            ...
            <table class="min-w-full divide-y divide-gray-300">
              <thead class="bg-gray-50">
                <tr>
                  ...
                  <th scope="col">
                    <input placeholder="All Colors" v-model="color" type="text" list="colors" class="pl-2 sm:pl-3 text-left text-xs sm:text-sm font-semibold text-gray-900 mt-1 block py-2 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-orange-500 focus:border-orange-500" />
                    <datalist id="colors">
                      <option v-for="(color, index) in uniqueColors" :key="index">{{ color }}</option>
                    </datalist>
                  </th>
...
export default {
    ...
    computed:{
      updatedVehicles(){
        ...
        if(this.color != '') v = v.filter(e => e.color.toLowerCase().match(new RegExp(this.color.toLowerCase + '.*')));
