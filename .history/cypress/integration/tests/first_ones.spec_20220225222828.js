...
function loginExampleCompany(){
    cy.get('#email').type('jayk@rollain.com');
    cy.get('#password').type('heyhey');
    cy.get('#register').click();
}

describe('login_company_page', () => {
    beforeEach(() => {
        cy.visit('/login-company');
    });
    it('can login and go to profile page', () => {
        registerExampleCompany();
        cy.contains('div', 'heybro');
    });
