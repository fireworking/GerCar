<template>
    <div class="min-h-full flex flex-col justify-center py-4 sm:py-16 sm:px-16 lg:px-20">
        ...
        <div class="bg-white py-2 sm:pb-4 sm:pt-1 px-4 shadow sm:rounded-lg sm:px-10">
          <form class="space-y-6" v-on:submit.prevent="submit" :action="currentUri" method="POST">
            ...
            <div v-if="data.includes('terms-of-service')">
              ...
              <span v-if="v.fields.termsOfService.$error && data.includes('terms-of-service')" class="text-red-500 text-sm">You must check the box to continue.</span>   
            </div>   
            <div>
...
<script>
export default {
    props: ['states', 'cities', 'defineCities', 'submit', 'fields', 'v', 'serverErrors', 'hrefSubtitle', 'data'],
    data() {
      return {
